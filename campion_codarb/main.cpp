...
ab *Arb(char*),*ArbSt(char*&),*ArbDr(char*&),*ArbSpec(char*&,ab*);
...
ab* ArbSpec(char* &pos,ab *fiu) {
	...
	if (*pos == 'H') {
		...
		pnt=ArbSpec(pos,sol);
		if(pnt!=NULL) {
		    //fprintf(stderr,"%c %c",pnt->inf,sol->inf);
			//pnt->fs=sol;
			return pnt;
		}
	}else {
...
ab* ArbSt(char* &pos) {
    ...
    if (*pos == 'E') {
		...
		if(pnt!=NULL) {
			//pnt->fs=sol;
			//aici e greseala
            //daca returnez cel mai de sus o sa lege frunza gresit, altfel o sa lege radacina gresit
			return pnt;
		}
    } else if (*pos == 'G') {
		sol->inf=*pos;
