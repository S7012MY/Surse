...
#define debd(n) fprintf(stderr,"%lf ",(n));
using namespace std;

typedef pair<double, double> per;

int n,x[DN],y[DN],dimx,dimy,djx,djy,rf,rind;
vector<per> sc,su;
vector<int> semn;
...
double solve(int p) {
    sc.clear(); semn.clear();
    sc.push_back(make_pair(djx,djy));
    sc.push_back(make_pair(djx,dimy));
    sc.push_back(make_pair(dimx,dimy));
    sc.push_back(make_pair(dimx,djy));
...
int main()
{
    ...
    x[n]=x[0]; y[n]=y[0];
    scanf("%d %d %d %d",&djx,&djy,&dimx,&dimy);
    for(int p=0; p<n; ++p) {
        double rez=solve(p);
        if(fabs(rez)<EPS) {
    ...
    printf("%d\n",rind);
	fprintf(stderr,"%lf ",(clock()-beg)/CLOCKS_PER_SEC);
    return 0;
}
