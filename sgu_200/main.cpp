...
void gauss() {
    for(int i=0; i<t; ++i) {
        int c;
        for(c=1; !s[i][c] && c<=m; ++c);// caut o coloana nevida
        if(c>m) continue;
        for(int j=i+1; j<t; ++j) if(s[j][c])
            for(int k=1; k<=m; ++k) s[j][k]^=s[i][k];
        --p;
    }
}
